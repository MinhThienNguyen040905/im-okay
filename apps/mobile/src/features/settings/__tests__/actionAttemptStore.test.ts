...
describe("settings action attempt store", () => {
  ...
  it("reuses one key per action until server confirmation", async () => {
    ...
    const first = await getOrCreateSettingsAttempt("user", "disable-plan", 1_000);
    expect(await getOrCreateSettingsAttempt("user", "disable-plan", 2_000)).toBe(first);
    await clearSettingsAttempt("user", "disable-plan");
    expect(await getOrCreateSettingsAttempt("user", "disable-plan", 3_000)).not.toBe(first);
  });
});
